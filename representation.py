"""
    ...
    Enconding and decoding of music
"""
import pathlib
import pprint
...
def advanced_decode(codes, encoding, soundfont="./data/MS_Basic.sf2", output_wav="output.wav"):

    music = decode(codes, encoding)
    
    # 2. Exportar a un archivo MIDI temporal
    # MusPy maneja esto de forma nativa y limpia
    temp_midi = "temp_render.mid"
    music.write(temp_midi)

    # Estructura: fluidsynth [OPCIONES] [SOUNDFONT] [MIDI]
    command = [
        'fluidsynth',
        '-ni',                      # No shell, no entrada MIDI
        '-a', 'none',               # Forzar SIN driver de audio (evita error de Pipewire/ALSA)
        '-T', 'wav',                # Formato de salida
        '-F', output_wav,           # Archivo de salida (DEBE ir antes de los archivos de entrada)
        '-r', '44100',              # Sample rate
        soundfont,                  # Archivo de sonido (entrada 1)
        temp_midi                   # Archivo MIDI (entrada 2)
    ]
...
def main():
    ...
    # --- NUEVA PARTE: GENERACIÓN DE AUDIO ---
