...
import dataset
# Importamos tu nueva clase (asegúrate de que el nombre del archivo sea correcto)
...
def parse_args(args=None, namespace=None):
    ...
    # Nota: 'dim' ahora debe coincidir con el hidden_size del LLM (Qwen 1.5B usa 1536)
    ...
    parser.add_argument("-lr", "--learning_rate", default=0.00005, type=float, help="learning rate (reducido para LLM)")
...
def compute_loss(logits_list, targets, mask, n_tokens):
    ...
    individual_losses = []
    for i in range(6):
        # Flatten para CrossEntropy: (Batch * Seq, Vocab)
        l_i = F.cross_entropy(
            logits_list[i].reshape(-1, n_tokens[i]), 
            targets[:, :, i].reshape(-1), 
            reduction='none'
        )
        # Aplicar máscara de padding
        masked_l_i = (l_i * mask.reshape(-1)).sum() / mask.sum()
...
def main():
    ...
    # Obtener n_tokens real del encoding
    n_tokens = encoding['n_tokens'] 
    logging.info(f"Vocabulario musical detectado: {n_tokens}")
    ...
    logging.info(f"Creating MusicLLM (Qwen-based)...")
    model = MusicLLM(
        model_name_or_path="Qwen/Qwen2.5-1.5B", 
        music_config={"n_tokens": n_tokens}
    ).to(device)

    # Activamos el freeze al inicio
    model.freeze_backbone(True)

    # Es importante recalcular los parámetros entrenables para el log
    n_trainables = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logging.info(f"Número de parámetros entrenables (Frozen): {n_trainables}")

    # El optimizador solo debe recibir parámetros que requieran gradiente (opcional pero recomendado)
    ...
    count_early_stopping = 0 if args.early_stopping else None
    ...
    while step < args.steps:
        ...
        # Guardar y Early Stopping (idéntico al original)
        loss_csv.write(f"{step},{train_loss},{val_loss}," + ",".join([f"{l}" for l in individual_losses]) + "\n")
        ...
        if val_loss < min_val_loss:
            ...
            if args.early_stopping: count_early_stopping = 0
